...
/**
 * A single candidate face bounding box produced by the [MTCNN] cascade.
 *
 * Ported verbatim from the MyEmotions source app (`data/local/mtcnn/Box.kt`) — the geometry math
 * (calibrate/toSquareShape/limitSquare) is part of the MTCNN algorithm and must not be altered.
 */
class Box {
    var box: IntArray = IntArray(4) // left:box[0],top:box[1],right:box[2],bottom:box[3]
    var score: Float = 0f // probability
    var bbr: FloatArray = FloatArray(4) // bounding box regression
    var deleted: Boolean = false
    var landmark: Array<Point?> = arrayOfNulls(5) // facial landmark
